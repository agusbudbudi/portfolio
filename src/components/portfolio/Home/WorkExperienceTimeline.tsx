...
const WorkExperienceTimeline: React.FC = () => {
  ...
  return (
    <section className="p-0 overflow-hidden relative">
      <div className="relative py-12 md:py-20 bg-ld-violet overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-4 relative z-10">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-10 gap-4">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 min-w-10 rounded-lg flex items-center justify-center mt-1 bg-white/20 text-white">
                <Briefcase size={20} />
              </div>
              <div className="flex flex-col gap-1">
                <h2 className="font-ld-display font-semibold text-2xl text-white m-0">
        ...
        <div className="overflow-x-auto scrollbar-none mt-12 timeline-scroll-container">
          <div className="flex gap-8 relative z-10 px-[max(24px,calc(50vw-580px))] w-fit">
            ...
            <div className="absolute top-[30px] left-[calc(max(24px,50vw-580px)+175px)] right-[calc(max(24px,50vw-580px)+175px)] h-0.5 bg-white/30 -z-10"></div>
            ...
            {experienceData.map((exp, index) => (
              <div key={index} className="w-[280px] sm:w-[300px] md:w-[350px] flex flex-col flex-shrink-0 group">
                <div className="w-full h-[60px] flex items-center justify-center relative">
                  <div className="w-5 h-5 bg-white border-4 border-ld-violet rounded-full z-10 shadow-[0_0_0_6px_rgba(255,255,255,0.3)] group-hover:scale-125 transition-transform duration-300"></div>
                </div>
                <div className="bg-ld-canvas border border-ld-ash rounded-xl p-4 md:p-6 w-full transition-colors flex flex-col gap-2 shadow-ld-subtle-2 group-hover:border-white">
                  <div className="flex gap-5 items-center">
                    <div className="w-[60px] h-[60px] min-w-[60px] bg-white rounded-lg flex items-center justify-center overflow-hidden shadow-sm">
                      <img src={exp.logo} alt={exp.company} loading="lazy" decoding="async" className="w-full h-full object-contain" />
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <h3 className="font-ld-display font-semibold text-base text-ld-graphite m-0 leading-tight">{exp.role}</h3>
                      <p className="text-sm text-ld-violet m-0 font-semibold">{exp.company}</p>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-4 border-t border-ld-ash mt-2">
                    <span className="text-xs text-ld-fog font-medium">{exp.period}</span>
                    <span className="text-[10px] bg-ld-lilac text-ld-violet px-3 py-1 rounded-full font-bold">
