...
interface TopicFormProps {
  onClose: () => void;
  onSubmit: (topic: TopicConfig) => Promise<{ ok: boolean; reason?: string }>;
...
const TopicForm: React.FC<TopicFormProps> = ({ onClose, onSubmit, topic, existingIds }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (!result.ok) {
      setError(result.reason ?? 'Gagal menyimpan topic.');
      return;
    }
    onClose();
  };
    ...
    <div className={ADMIN_CARD}>
      ...
      <form onSubmit={handleSubmit} className={`${ADMIN_CARD_BODY} w-full`}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="pt-6 lg:pt-0 border-t lg:border-t-0 lg:border-l border-ld-frost/60 lg:pl-8">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {form.image && !imageBroken ? (
                    <img
                      ...
                      className="w-12 h-12 rounded-lg object-cover border border-ld-frost shrink-0"
                    />
                  ) : (
                    <span className="w-12 h-12 rounded-lg bg-ld-cloud border border-ld-frost shrink-0 flex items-center justify-center text-[10px] text-ld-fog">
                      {imageBroken ? 'rusak' : '—'}
                    </span>
                  )}
                </div>
        ...
        <div className="flex justify-end gap-2 mt-8 pt-5 border-t border-ld-frost/60">
          ...
          >
            {saving ? 'Menyimpan…' : isEdit ? 'Simpan Topic' : 'Tambah Topic'}
          </button>
        </div>
      </form>
    </div>
