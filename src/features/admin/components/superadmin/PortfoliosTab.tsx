...
const STATUS_TONE: Record<PortfolioSummary['status'], StatusBadgeTone> = {
  draft: 'slate',
  published: 'emerald',
};
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-ld-cloud text-left">
                    ...
                    <th className="px-4 py-3 text-xs font-medium text-ld-steel whitespace-nowrap">Owner</th>
                ...
                <tbody>
                    ...
                    <tr key={portfolio.id} className="border-t border-ld-frost bg-white hover:bg-ld-cloud/50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3 min-w-[180px]">
                          <img
                            src={portfolio.photo || '/saas-portfolio/img/portfolio-public/default-avatar-portfolio.webp'}
                            alt=""
                            className="w-9 h-9 rounded-lg object-cover border border-ld-frost shrink-0"
                          />
                          <button
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {portfolio.ownerId ? (
                          <code className="text-[11px] text-ld-slate">{portfolio.ownerId}</code>
                        ) : (
                          <span className="text-xs text-ld-fog italic">—</span>
                        )}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <StatusBadge tone={STATUS_TONE[portfolio.status]}>{portfolio.status}</StatusBadge>
                      </td>
                      <td className="px-4 py-3 text-xs text-ld-slate whitespace-nowrap">
                        {new Date(portfolio.updatedAt).toLocaleDateString('id-ID')}{' '}
                        {new Date(portfolio.updatedAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                      </td>
                      <td className="px-4 py-3">
                  ...
                  {portfolios.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-6 text-center text-sm text-ld-fog">Belum ada portfolio.</td>
                    </tr>
                  )}
                </tbody>
              </table>
