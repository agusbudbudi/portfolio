...
const BookingsTab: React.FC = () => {
  ...
  const {
    bookings, loading, loadError, load, create, update, transitionBooking,
  ...
  if (formOpen) {
    ...
    return (
      <AdminBookingForm
        ...
        onSubmit={(booking) => (editing ? update(editing.id, booking) : create(booking))}
        booking={editing}
        mentors={mentors}
        topics={topics}
                ...
                <tr key={booking.id} className="border-t border-ld-frost bg-white hover:bg-ld-cloud/50 transition-colors">
                  <td className="px-4 py-3 align-top whitespace-nowrap">
                    <button
                      ...
                      className="text-xs font-mono uppercase text-ld-violet hover:underline cursor-pointer border-none bg-transparent p-0"
                  ...
                  <td className="px-4 py-3 align-top whitespace-nowrap">
                    <StatusBadge tone={STATUS_TONE[booking.status]}>{STATUS_LABEL[booking.status]}</StatusBadge>
                  </td>
                  <td className="px-4 py-3 align-top whitespace-nowrap">
