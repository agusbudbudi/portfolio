...
import {
  ...
  UserCircle2, Briefcase, GraduationCap, Award, FolderKanban, Quote, Newspaper, Share2, MousePointerClick,
} from 'lucide-react';
...
import EndorsementListEditor from './EndorsementListEditor';
import CertificationListEditor from './CertificationListEditor';
...
const PortfolioForm: React.FC<PortfolioFormProps> = ({ onClose, onSubmit, record, tools }) => {
    ...
    <div className={ADMIN_CARD}>
      <div className={ADMIN_CARD_HEADER}>
        ...
        <div className="flex items-baseline gap-2 min-w-0">
          ...
          {isEdit && (
            <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium uppercase tracking-widest shrink-0 ${STATUS_BADGE[record.status]}`}>
              {record.status}
            </span>
          )}
      ...
      <form ref={formRef} onSubmit={handleSubmit} className={`${ADMIN_CARD_BODY} w-full space-y-3`}>
        <AccordionSection title="Profile" icon={UserCircle2} hasError={profileHasError} errorSignal={submitAttempt}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4">
            <ImageUploadField
              ...
              shape="rounded-md"
              fallbackSrc="/img/portfolio-public/default-avatar-portfolio.webp"
            />
